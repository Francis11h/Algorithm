...
Mysql存储引擎是？(多存储引擎机制)
        ...
        默认的存储引擎: InnoDB
            1."更新密集"的表。                       InnoDB存储引擎特别适合处理"多重并发"的更新请求。
            2.事务。                             InnoDB存储引擎是支持事务的标准MySQL存储引擎。
            3.自动灾难恢复。与其它存储引擎不同       InnoDB表能够自动从灾难中恢复。
            4.外键约束。                         MySQL"支持外键"的存储引擎只有InnoDB。
        ...
        MyISAM多用于
            "选择密集型"的表, MyISAM存储引擎在 筛选大量数据时  非常迅速，这是它最突出的优点。
            "插入密集型"的表, MyISAM的 并发插入特性 允许同时选择和插入数据。
                例如：MyISAM存储引擎很适合管理 邮件或Web服务器日志数据。

        "MyISAM 引擎不支持事务"的概念, 多用于数据仓库这样 查询多而事务少 的情况, 速度较快。
...
什么是事务
        ...
        一个数据库事务通常包含对数据库进行读或写的一个"操作序列 即多条指令"
        事务存在的意义
...
事务四大特性是什么？
        ...
        隔离性（Isolation）:
            多个事务"并发"执行时，一个事务的执行不应影响其他事务的执行。
        持久性（Durability）:
...
事务的ACID是通过InnoDB日志log和锁来lock保证

    事务的隔离性(多个事务"并发"执行时，一个事务的执行不应影响其他事务的执行。)是通过数据库"锁"的机制实现的，
    持久性(一个事务一旦提交，他对数据库的修改应该永久保存在数据库中)通过"redo log（重做日志）"来实现，
    原子性和一致性(事务作为一个整体被执行 + 事务应确保数据库的状态从一个一致状态转变为另一个一致状态) 通过 "Undo log"来实现。
    ...
    和Undo Log相反，RedoLog记录的是新数据的备份。
    在事务提交前，只要将RedoLog持久化即可，不需要将数据持久化。
    当系统崩溃时，虽然数据没有持久化，但是RedoLog已经持久化。
    系统可以根据RedoLog的内容，将所有数据恢复到最新的状态。


数据库的乐观锁和悲观锁是什么？数据库的悲观锁和乐观锁是啥有什么区别
        乐观并发控制(乐观锁)和悲观并发控制 Pessimistic Concurrency Control（悲观锁）是"并发Concurrent"控制主要采用的技术手段。

        悲观:
            如果一个事务执行的操作都某行数据应用了锁，那"只有当这个事务把锁释放，其他事务才能够执行与该锁冲突的操作"。
            悲观并发控制主要用于 数据争用激烈的环境 + 以及 发生并发冲突时 使用锁保护数据的成本 < 回滚undo事务的成本 的环境中。
            往往依靠数据库提供的锁机制 
        ...
        乐观:    
            乐观锁假设认为数据一般情况下不会造成冲突，所以"在数据进行 提交更新 的时候，才会正式对数据的冲突与否进行检测"，
