...
同步调用与异步调用什么是同步调用？
----------------------------
一句话: 异步调用 client 不用等 server返回的结果 先干别的事儿 过一会儿收到通知再回来取结果

同步调用就是	客户端等待调用执行完成并返回结果。
异步调用就是	客户端"不等待调用执行完成返回结果"，不过依然可以通过"回调函数callback"等接收到返回结果的通知。

如果客户端并不关心结果，则可以变成一个单向的调用。
这个过程有点类似于Java中的callable和runnable接口，
我们进行异步执行的时候，如果需要知道执行的结果，就可以使用callable接口，并且可以通过Future类获取到异步执行的结果信息。
如果不关心执行的结果，直接使用runnable接口就可以了，因为它不返回结果，当然啦，callable也是可以的，我们不去获取Future就可以了。


更本质一些 是谁帮我们干完的活儿 从而直接给我们结果？

第一种情况，本地IO操作时，可以通过DMA功能实现，在调用DMA传输数据的时候，CPU是不需要执行处理的，
	只需要发起传输和等待传输即可，也就是说，在这段时间里，CPU可以干点别的事情。
	DMA的英文拼写是“Direct Memory Access”， 就是直接内存访问。
	DMA既可以指内存和外设直接存取数据这种内存访问的计算机技术，
	又可以指实现该技术的硬件模块（对于通用计算机PC而言，DMA控制逻辑由CPU和DMA控制接口逻辑芯片共同组成，嵌入式系统的DMA控制器内建在处理器芯片内部，一般称为DMA控制器，DMAC）。

第二种情况，通过多线程实现，主线程发起请求操作（这里用线程解释，多进程也是可以的），
	系统选取一个线程接过这个主线程的请求任务，
	然后当异步调用完成后，系统会从可用线程中选取一个线程执行回调函数，将结果推回给主线程。
	这里的异步调用，主要是为了让调用方法的"主线程不需要同步等待在这个函数调用上"，从而可以让主线程继续执行它下面的代码。
