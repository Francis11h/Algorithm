...
阻塞/非阻塞 同步/异步
--------------------
阻塞和非阻塞可以理解为是"发送方"的行为，同步和异步是"接收方"的行为 本质上是一样的

进程间的通信时通过 send() 和 receive() 两种基本操作完成的。
...
也就是说，阻塞和非阻塞可以理解为是发送方的行为，同步和异步是接收方的行为。

举个例子：我们现在去在一条步行街上的银行办理业务，根据场景可以解释为以下多种情况：
