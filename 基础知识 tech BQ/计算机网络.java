...
SYN: Synchronize 标记的包, 一个 SYN包就是仅SYN标记设为1的TCP包
FIN: Finall
RST: Reset重连位

如果建立连接时 只"两次"握手就建立了 会有什么问题？

但是server无法知道client是否已经接收到自己的同步信号，如果这个同步信号(server给client的SYN)丢失了，
A和B就B的初始序列号将无法达成一致。


为什么建立连接是三次握手，关闭连接确是"四次"挥手呢？
...
------------------------
"TIME_WAIT" 这个状态 为什么必须
------------------------
    假设最终的ACK丢失，server 将重发FIN，client 必须维护TCP状态信息以便可以重发最终的ACK，否则会发送RST，
    结果server 认为发生错误 会一直重复发送 FIN
    TCP实现必须可靠地终止连接的两个方向(全双工关闭)，client 必须进入 TIME_WAIT 状态，因为 client 可能面 临重发最终ACK的情形。

    人话:  1. 防止server再重传
    	  2. 防止 道路上还有原server没传完的包 此时若有新的连接 会数据错乱
