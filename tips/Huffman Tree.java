...
prefix code :no codeword can be the prefix of some other codeword

root-to-leaf will determine the code


encoding

    1.每次总取频率最小的两个点，组合成新点
    2. less frequent characters will be the deeper in their trees
        即频率越大越靠近跟节点
    3.建完树后 标记上0, 1   left = 0， right= 1.
    每条树的(root 到 leaf)路径都是一个字符
...
build huffman tree
    ...
    H = A min-heap with a node for each character sorted by frequency   O(n)

    while H contains 2 or more elements                 O(n) iterations
        x = extractMin(H);                              O(nlogn)
        y = extractMin(H);                              O(nlogn)
        z = make a new parent for x and y
        z.freq = x.freq + y.freq;
        inzert z into H                                 O(nlogn)
...
具体实例
https://blog.csdn.net/kimylrong/article/details/17022319
